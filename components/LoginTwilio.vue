        ...
        <v-dialog v-model="activeModalLogin" persistent content-class="ma-0 content-login" width="360px">
            <v-form ref="form" :v-model="valid" lazy-validation>
                <v-card class="rounded-lg mx-auto" style="background-color: #ffffffe6" width="100%">
                    <v-card-text class="pa-5" style="padding-bottom: 0 !important;">
                        <div class="header">
                            <div @click="showCode">
                        ...
                        <div v-show="phoneShow" class="phoneShow">
                            ...
                            <vue-phone-number-input size="lg" id="importan_input" default-country-code="US"
                                @update="onUpdate" v-model="phone" :error="!results.isValid" />
                            <v-btn block class="rounded-lg py-4 mt-6 text-capitalize buttomLogin" style="
                    height: 50px;
                    display: flex;
                    margin-top: 10px;
                    background-color: #33a9f2;
                  " color="primary" :disabled="!results.isValid || !checKTerms" @click="sendPhone()">
                                <div style="
                      display: flex;
                    ...
                    " v-show="loadingStatus">
                            ...
                            <label class="terms">
                                <input style="margin-top: 20px;" v-model="terms" type="checkbox" id="cbox1"
                                    value="first_checkbox" />
                                I agree to receive messaging regarding my Shareity account at the phone number above. I
                                understand I can opt out at any time, Msg & data rates may apply, reply STOP to opt out.

                            </label>
                            <div
                                style="display: flex; justify-content: center; align-items: center; gap: 5px; padding: 20px; padding-bottom: 0;">
                                <p style="color: #33a9f2;" @click="$nuxt.$emit('UseTerms4', true)"> Privacy Policy </p>
                                <p>|</p>
                                <p style="color: #33a9f2;" @click="$nuxt.$emit('UseTerms3', true)">Terms of Use </p>
                            </div>
                        </div>
                        <div v-show="codeShow" class="codeShow">
                            ...
                            <v-text-field v-show="infoLogin" label="Name" hide-details="auto"
                                v-model="nameUser"></v-text-field>
                  ...
                  max="6"></v-text-field> -->
                    ...
                    <v-card-actions class="cardInfo">
                        ...
                        <v-img class="ml-2" style="width: 30px; height: 30px; align-self: flex-end"
                            @click="$nuxt.$emit('UseTerms1', true)"
                            :src="require('~/assets/img/svg/info_icon_blue.svg')"></v-img>
                    </v-card-actions>
                </v-card>
            </v-form>
...
export default {
            ...
            if (this.completeChallenge === "noPromotional" && value <= 0) {
                setTimeout(() => {
                    ...
                    this.$router.push({
                        ...
                        query: { shared: true, u: this.resId.id, id: this.challengeRes.id },
                    });
                }, 800);
            }
        ...
        sendPhone() {
            ...
            this.setLoginFrom({
                ...
                if (!resp.isNew) {
                    ...
                    this.setVerifyCode({ number: this.infoPhoneNumber, code: resp.code }).then((res) => {
                        console.log('res', res)
                        location.reload();
    ...
    created() {
        this.$nuxt.$on("openLoginTwilio", (option) => {
            this.nicknameAction().then((user) => {
                this.userId = user.id;
            });
            document.getElementsByTagName("body")[0].style.overflow = "hidden";
            this.countDown =
                option[2]?.challenge?.sponsors[0]?.animation?.duration / 1000 ||
                option[2]?.sponsors[0]?.animation?.duration / 1000;
            this.from = option[4];
