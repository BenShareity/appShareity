  ...
  <v-card outlined shaped elevation="4" class="mx-auto" color="#fff" dark max-width="400">
    <v-card-actions class="shadow__">
      <v-list-item class="grow__edit">
        <div style=" width: 65px; height: 65px; border-radius: 8px "
          @click="$router.push({ path: `/profile/${item.username}` })">
          <img v-if="item.ImgUsr !== '' && item.ImgUsr !== '-' && item.ImgUsr !== null" class="img__classEdit" alt=""
            :src="urlBucket + item.ImgUsr
            " />
          <svg v-else class="img__classEdit" width="50" height="50" viewBox="0 0 50 50" fill="none"
        ...
        <v-list-item-content>
          ...
          <!-- <p class="editTextUser">{{ item.FnameUsr }}</p> -->
          <!-- <div class="editTextG">THIS CAUSE $5</div>
              <div class="editTextG">ALL CAUSES $34</div>
              <div class="editTextG">SWAY $36</div> -->
        </v-list-item-content>

        <v-row class="none_flex" align="center" justify="end">
          <v-btn v-if="item.status === 0" class="color_button" color="#33A9F2" @click="addFriend()">
            Add friend
          </v-btn>
          <v-btn v-else-if="item.status === 2" class="color_button" color="#6f6f6f">
            Pending
          </v-btn>
          <v-btn v-else-if="item.status === 3" class="color_button" color="#33A9F2" @click="addFriend()">
            Accept request
          </v-btn>
          <v-btn v-else class="color_button" color="#4caf50" @click="addFriend()">
            Chat
          </v-btn>
        </v-row>
      </v-list-item>
...
<script>
import logoS from "../assets/img/png/userfeed.png";
import { mapActions } from "vuex";
export default {
  props: ["item", "session", "friends"],
  data() {
    return {
      imgNone: logoS,
    };
      ...
      if (this.item.status == 0) {
        this.$axios
          .$post("/user/friendship-request", {
            ...
            userFriendship: [Number(this.item.id)],
          })
          .then((res) => {
        ...
        this.$router.push({ path: `/chat/${this.item.username}` });
