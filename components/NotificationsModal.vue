<template>
  <v-row justify="center">
    <v-dialog v-model="show" transition="dialog-bottom-transition" :width="windowWidthT + 14" :content-class="`${toListenWidth > 1264 && toListenWidth < 1904 ? 'margin_desptop' : ''
      }`">
      <v-card>
        <v-toolbar dark color="#33A9F2" height="50">
          <v-btn icon dark @click="closetNotification">
          ...
          <!-- <v-toolbar-items>
            <div class="container_IMG">
              <a href="/notification/setting">
                <img class="T_img" style="height: 33px; width: 22px; margin: 0px 0px 0px 20px" :src="imgSetting" />
              </a>
            </div>
            <div class="container_IMG">
              <img class="T_img" style="height: 33px; width: 22px; margin: 0px 0px 0px 20px" :src="imgLink" />
            </div>
          </v-toolbar-items> -->
        </v-toolbar>
...
const imgSetting = require("@/assets/img/png/footer/icoLogo.png");
...
export default {
  ...
  created() {
    this.$nuxt.$off("openModalNotifications");
    this.$nuxt.$on("openModalNotifications", (option) => {
