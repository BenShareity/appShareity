<template>
  <div class="text-center containerComponent">
    <v-dialog v-model="activeShared" max-width="400" persistent content-class="no-scroll" :content-class="`${toListenWidth > 1264 && toListenWidth < 1904 ? 'margin_desptop' : ''
      }`">
      <v-sheet class="text-center height_modal" style="position: relative">
        <div v-if="donate" class="donation-modal">
          <div v-if="loadingDonate"
            style="height: 400px; display: flex; gap: 10px; flex-direction: column; justify-content: center; align-items: center">
            <LoadingIos />
            <p class="donation-modal-contend_p-1" style="text-transform: capitalize; font-size: 10px !important;">
              loading donation...</p>
          </div>
          <template v-else>
            <div v-if="checkDonation"
              style="display: flex; gap: 5px;padding: 20px; flex-direction: column; justify-content: center; align-items: center;">
              <p class="donation-modal-contend_p-1" style="text-transform: capitalize;">Thank you</p>
              <p class="donation-modal-contend_p-2" style="text-transform: capitalize;">for your
                donation!</p>
              <p class="donation-modal-contend_p-3">When you are approved you will become this
                challenge
                sponsor!</p>
            ...
            <template v-else>
              <div class="donation-modal-contend">
                <p class="donation-modal-contend_p-1">This challenge needs</p>
                <p class="donation-modal-contend_p-2">A SPONSOR</p>
                <p class="donation-modal-contend_p-3">You can become a sponsor
                  by making a
                  donation or skip to just promote awereness</p>
              </div>
              <div class="donation-modal-amount">
                <div @click="postDonation(5)" class="donation-modal-amount_button">
                  <p class="donation-modal-amount_p">$5</p>
                </div>
                <div @click="postDonation(10)" class="donation-modal-amount_button">
                  <p class="donation-modal-amount_p">$10</p>
                </div>
                <div @click="postDonation(20)" class="donation-modal-amount_button">
                  <p class="donation-modal-amount_p">$20</p>
                </div>
              </div>
            </template>

            <div>
              <img v-if="checkDonation" :src="steveThanks" alt="">
              <img v-else :src="steveDonate" alt="">
            </div>
            <div class="donation-modal-skip">
              <p @click="donate = false">
                <span v-if="checkDonation">Done</span>
        ...
        <template v-else>
          ...
          <v-bottom-sheet hide-overlay persistent v-model="QrClicked" max-width="414px" :retain-focus="false">
            <v-sheet class="text-center container-modal-2" style="background-color: #1976d2" height="260px">
              ...
              <div class="py-3 container-modal__section2">
                <div class="container-modal__section2__img">Z<!--  -->
                  <img src="@/assets/img/png/steve_buy_usd.png" alt="" />
                </div>
                <div class="container-modal__section2__item">
                  <Qr class="" :textSrc="urlLink" :badgeImage="itemData?.challenge?.badge"
                    :badgeColor="itemData?.challenge?.color" />
                </div>
              </div>
            </v-sheet>
          </v-bottom-sheet>
              ...
              <div class="py-3 container-modal__intagram__section2">
                ...
                <div class="contend-social-is">
                  ...
                  <div class="social-is" style="margin: 0">
                    ...
                    <p @click="getDownload(itemData)" class="text_pp">Download Your Challenge Video</p>
                  </div>
                  ...
                  <div v-if="iconCopyLink.from === 0" class="description">
                    Create a Story and direct friend to accept your challenge by clicking your link. Add Your Challenge
                    Link
                    via
                  ...
                  <div v-else class="description">
                    ...
                    Challenge
                    Link url. Paste your challenge link in the comment section of your TikTok video. Encourage viewers
                    to
                    copy
      ...
      " target="_blank">
          ...
          <div class="class__butoon__">
            <div v-if="itemData" class="div-img">
             <!--  <div class="bg-clue-custom"></div> -->
              <video v-if="itemData?.resource?.id === 3" @click="playVideo()"
                :class="showVideo ? 'no-video' : 'video-show '" playsinline preload="auto" ref="videoComminities" muted
                :poster="urlBucket + itemData?.watermark" :src="urlBucket + itemData?.url">
              </video>
              <img :class="showVideo || itemData?.resource?.id !== 3 ? 'imagen-show ' : 'no-image'"
                :src="urlBucket + itemData?.watermark" alt="">
            </div>
            <div class="share-title">
              <div>Now share your
                <p style="font-size: 12px;  white-space: pre; color: #449AD9;">Challenge link to raise more!</p>
              </div>
            </div>
              ...
              <div class="center-money">
                <div class="button-mnumber" :style="[active1 && { color: 'white', background: '#00a5ff' }]" medium
                  ...
                  <p class="donation-text" :class="active1 ? 'donation-text-white' : 'donation-text-green'"
                    v-if="itemData?.challenge?.typeDonation == 'money'"> ${{ itemData?.challenge?.amount }}</p>
                  <img style="width: 100%; padding: 5px; height: auto;" v-else
                    :src="urlBucket + itemData?.challenge?.typeDonationIcon" alt="">
                </div>
              ...
              <div class="center-money">
                <div class="button-mnumber" :style="[active2 && { color: 'white', background: '#00a5ff' }]" medium
                  ...
                  <p class="donation-text" :class="active2 ? 'donation-text-white' : 'donation-text-green'"
                    v-if="itemData?.challenge?.typeDonation == 'money'"> ${{
      itemData?.challenge?.amount }}</p>
                  <img style="width: 100%; padding: 5px; height: auto;" v-else
                    :src="urlBucket + itemData?.challenge?.typeDonationIcon" alt="">
      ...
      itemData?.challenge?.amount }}</p>
          ...
          <div id="style-item" class="social-items">
            <div v-if="itemData?.resource?.id === 3" class="social-item">
            ...
            <div class="social-item" @click="sharedSocial('facebook')">
              <ShareNetwork network="facebook" :url="urlFaceebok" :title="itemData?.challenge?.shareMessage || ''"
                hashtags="Shareity">
            ...
            <div class="social-item" @click="sharedSocial('twitter')">
              <ShareNetwork network="twitter" :url="urlTwitter" :title="itemData?.challenge?.shareMessage || ''"
                hashtags="Shareity">
            ...
            <div class="social-item" @click="sharedSocial('whats')">
              <ShareNetwork network="whatsApp" :url="urlWhatsApp" :title="itemData?.challenge?.shareMessage || ''"
                hashtags="Shareity">
            ...
            <div class="social-item" @click="sharedSocial('pinterest')">
              <ShareNetwork network="pinterest" :url="urlPinterest" v-if="itemData"
                :media="urlBucket + itemData.watermark" :title="itemData?.challenge?.shareMessage || ''"
                hashtags="Shareity">
            ...
            <div class="social-item" @click="sharedSocial('email')">
              <div class="img__social">
                <a :href="`mailto:?&body=${itemData?.challenge?.shareMessage + '  ' + urlEmail
      }`" target="_blank">
            ...
            <template v-if="itemData">
              <div class="social-item" style="margin: 0">
                <img class="img__social" @click="getDownload(itemData)" src="@/assets/img/png/social/down-arrow.png" />
          ...
          <div class="footer-shared">
            ...
            <div class="footer-shared-done" @click="clickedShared()">Done</div>
          </div>
...
import steveDonate from '../../assets/steve-donate.svg'
import steveThanks from '../../assets/thenksDonation.svg'
export default {
  data() {
    return {
      url: "",
  ...
  methods: {
    ...
    sharedSocial(string) {
      if (this.itemData?.resource?.id === 3) {
    ...
    counter() {
      window.open(
        `sms:?&body=${this.itemData.challenge.shareMessageShort + "  " + this.urlSms
        }`,
        "_blank",
    ...
    copy(string, shared) {
      ...
      /* this.getDownload(this.itemData) */
    },
    copyUrlLink() {
      this.showVideo = true
      var dummy = document.createElement("input");
    ...
    modalCopyLink(from) {
      if (from === "instagram") this.iconCopyLink = { status: true, from: 0 };

      if (from === "tiktok") this.iconCopyLink = { status: true, from: 1 };
    },
    playVideo(item) {
    ...
    postDonation(amount) {
      ...
      this.$axios.$post("/pledges/createPledge/user", {
        amount: amount,
        idChallenge: this.itemData.challenge.id,
        idUser: this.userData.id
      }).then((res) => {
        var link = document.createElementNS("http://www.w3.org/1999/xhtml", "a");
        link.href = this.itemData.challenge.organization.donationLink;
        link.target = '_blank';
      ...
      /*   window.open(this.itemData.challenge.organization.donationLink, '_blank'); */
        this.loadingDonate = false;
        this.checkDonation = true
        console.log('res', res)
      }).catch((err) => {
        console.log("err", err);
        this.loadingDonate = false;
  ...
  created() {
    if (this.$route.query.shared) {
      console.log('this.$route.query.donate', this.$route.query.donate)
      this.loadingShared = true;
      this.$axios
        .$get("/user-challenge/get-by-id/" + this.$route.query.id)
        .then((res) => {
          console.log('res', res)
          this.itemData = res;
          this.$axios
            .$post("/stats/UserChallenge/GetShare", {
              userChallengeId: res.id,
              platform: [
                "Twitter",
                "Facebook",
                "Instagram",
            ...
            .then((resShared) => {
              ...
              this.messageSend = this.itemData?.challenge?.shareMessageShortIG;
              this.tiktokLink = `${window.location.origin}/userChallenge/shares/${resShared.TikTok.id}/${this.$route.query.id}`;
              this.InstagramLink = `${window.location.origin}/userChallenge/shares/${resShared.Instagram.id}/${this.$route.query.id}`;
              this.urlFaceebok = `${window.location.origin}/userChallenge/shares/${resShared.Facebook.id}/${this.$route.query.id}`;
              this.urlTwitter = `${window.location.origin}/userChallenge/shares/${resShared.Twitter.id}/${this.$route.query.id}`;
              this.urlPinterest = `${window.location.origin}/userChallenge/shares/${resShared.Pinterest.id}/${this.$route.query.id}`;
              this.urlWhatsApp = `${window.location.origin}/userChallenge/shares/${resShared.Whatsapp.id}/${this.$route.query.id}`;
              this.urlLink = `${window.location.origin}/userChallenge/shares/${resShared.Direct.id}/${this.$route.query.id}`;
              this.urlText = `${window.location.origin}/userChallenge/shares/${resShared.Direct.id}/${this.$route.query.id}`;
              this.urlEmail = `${window.location.origin}/userChallenge/shares/${resShared.Email.id}/${this.$route.query.id}`;
              this.urlIntagram =
                this.itemData?.challenge?.shareMessageShortIG ? this.itemData?.challenge?.shareMessageShortIG + " " + `${window.location.origin}/userChallenge/shares/${resShared.Instagram.id}/${this.$route.query.id}` : 'Shareity' +
                  "  " +
                  `${window.location.origin}/userChallenge/shares/${resShared.Instagram.id}/${this.$route.query.id}`;
              this.urlTikTok =
                this.itemData?.challenge?.shareMessageShortIG ? this.itemData?.challenge?.shareMessageShortIG + " " + `${window.location.origin}/userChallenge/shares/${resShared.Instagram.id}/${this.$route.query.id}` : 'Shareity' +
                  "  " +
                  `${window.location.origin}/userChallenge/shares/${resShared.TikTok.id}/${this.$route.query.id}`;
              this.urlSms = `${window.location.origin}/userChallenge/shares/${resShared.Direct.id}/${this.$route.query.id}`;
              setTimeout(() => {
                this.loadingShared = false;
                console.log('this.itemData.challenge', this.itemData.challenge)
                if (this.itemData.challenge.organization.donationLink && this.itemData.challenge.sponsors.length === 0) {
                  /*    localStorage.setItem('donate', 0) */
                  this.donate = true
                  /*  if (this.$route.query.donate == 'true') this.donate = true */
                }
  ...
  components: { LoadingIos },
};
</script>
<style>
.donation-modal .lds-spinner div:after {
  background: #13a7de !important;
}

/* .container-communities2 .v-overlay__scrim {
...
.donation-modal-contend_p-1 {
  font-size: 20px;
  color: #545454;
  font-weight: 700;
}

.donation-modal-contend_p-2 {
  color: #2F4B75;
  font-size: 2rem;
  font-weight: 600;
}

.donation-modal-contend_p-3 {
  font-size: 20px;
  text-align: center;
  color: #545454;
  font-weight: 700;
...
.donation-modal-skip {
  width: 100%;
  text-align: center;
  border-top: 1px solid #757575B2;
  padding: 20px;
}

.donation-modal-skip>p {
  color: #1976d2;
  font-weight: 600;
}
