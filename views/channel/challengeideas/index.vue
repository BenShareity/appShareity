<template>
  <v-row justify="center">
    <div>
      <v-container class="edit-mdl" v-if="getQuizList">
        <v-row no-gutters align="center" class="aligment">
          <v-col col-md-3>
            <v-row no-gutters>
              <div
                class="tube"
                v-for="(quizOptions, index) in getQuizList.quizOptions"
                :key="index"
              >
                ...
                <v-progress-linear
                  class="rotate"
                  height="22px"
                  v-model="quizOptions.percentage"
                ></v-progress-linear>
        ...
        <v-container class="px-0" v-if="getQuizList">
          <div
            v-for="(quizOptions, index) in getQuizList.quizOptions"
            :key="index"
          >
            <v-row
              ...
              :style="
                index % 2 === 0 ? 'background: white;' : 'background: #D9DCE1;'
              "
            >
              <v-col class="col-1 num-item"> #{{ quizOptions.id }} </v-col>
                ...
                <v-img
                  ...
                  :src="
                    quizOptions.selected
                      ? require('~/assets/img/svg/channel/challenge/heart-blue.svg')
                      : require('~/assets/img/svg/channel/challenge/heart-gray.svg')
                  "
                ></v-img>
              </v-col>
            </v-row>
          ...
          <v-row
            v-if="isVote && !isQuizVote"
            no-gutters
            justify="center"
            style="margin: 20px 0px 40px"
          >
            <elements-buttons-buttonsvotationact @click="vote()" text="vote" />
...
export default {
  ...
  computed: {
    ...
    isVote() {
      return (
        this.getQuizList.quizOptions.findIndex((v) => v.selected === true) > -1
      );
    },
  },
  methods: {
    ...
    vote() {
      let quiz = this.getQuizList.quizOptions.filter(
        (v) => v.selected === true
      );
      let data = {
        idUsr: "14180",
